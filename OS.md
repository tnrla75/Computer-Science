...
P1이라는 프로세스가 A라는 값을 이용해 로직을 수행중
P2라는 프로세스가 A의 값을 3으로 바꿀경우 답이 다르게 나올것이다.

A를 여기서 공유 자원이라 하고
여기서 두개 이상의 프로세스 혹은 스레드가 동기화 없이 접근하는 현상을 race condition 라고 하며
이 문제를 해결하기 위해, 알고있는 정보를 일치시키는 것을 동기화 라고 한다.
```
- IPC 통신
```
프로세스들 간의 의사소통 하는것을 IPC라 한다.

커널 영역에서 IPC라는 내부 프로세스간 통신을 제공하여 프로세스끼리 의사소통이 가능하다.

크게 메시지 전달(Message Passing), 메모리 공유(Shared Memory)가 있다.
```
